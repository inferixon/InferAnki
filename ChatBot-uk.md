...
Спільна модель налаштовується один раз у `config.json` через `openai_default_model`. Окремі промпти можуть перевизначити її лише тоді, коли їм справді потрібна спеціалізована модель.
...
Типовий пресет містить `BETYDNINGEN`, `FORSKJELLEN`, `SYNONYMER`, `BERGENSK`, `PROOFREAD` і переклад норвезькою, англійською та українською.
...
`max_completion_tokens`, `copy_to_clipboard` і винятковий override `model` – необов’язкові налаштування окремого промпту. Спільна модель лишається в `config.json`.
...
3. **Встановлюйте відповідні токен-ліміти**: Короткі відповіді = 400-800, детальні = 1000-1500, для довгих текстів (наприклад, Proofread) використовуйте 3000–7000.
